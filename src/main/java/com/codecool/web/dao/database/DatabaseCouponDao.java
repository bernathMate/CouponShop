...
public final class DatabaseCouponDao extends AbstractDao implements CouponDao {
    ...
    @Override
    public Coupon findByIdAndUserId(int id, int userId) throws SQLException {
        String sql = "SELECT id, name, percentage, user_id FROM coupons WHERE id = ? AND user_id = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, id);
    ...
    @Override
    public List<Coupon> findAllByUserIdAndShopId(int userId, int shopId) throws SQLException {
        List<Coupon> coupons = new ArrayList<>();
        String sql = "select coupons.id, coupons.name, coupons.percentage, coupons.user_id from coupons " +
            "join coupons_shops on coupons_shops.coupon_id = coupons.id " +
            "join shops on shops.id = coupons_shops.shop_id " +
            "where coupons.user_id = ? and shops.id = ?;";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, userId);
